...
export function Calculator() {
  ...
  const handleCalculate = () => {
    if (selectedTipoBem === "") {
      const notifyWarning = () => toast("Selecione o tipo de bem.", {type: "warning"});
      notifyWarning();
      return;
    }

    if (selectedEstadoName === "") {
      const notifyWarning = () => toast("Selecione o estado.", {type: "warning"});
      notifyWarning();
      return;
    }

    if (selectedCidade === "") {
      const notifyWarning = () => toast("Selecione a cidade.", {type: "warning"});
      notifyWarning();
      return;
    }

    if (selectedDataAquisicao === "") {
      const notifyWarning = () => toast("Selecione a data de aquisição.", {type: "warning"});
      notifyWarning();
      return;
    }

    if (userInput === 0) {
      const notifyWarning = () => toast("Insira o valor venal.", {type: "warning"});
      notifyWarning();
      return;
    }

    if (selectedValorMercado === "") {
      const notifyWarning = () => toast("Insira o valor de mercado.", {type: "warning"});
      notifyWarning();
      return;
    }

    if (localStorage.length >= 5){
      const notifyWarning = () => toast("Numero maximo de registros atingido.", {type: "warning"});
      notifyWarning();
      return;
    }

    const result = (parseFloat(userInput) / selectedEstadoValue).toFixed(2).toString();
    const notifySucess = () => toast("Foi inserido sucesso!", {type: "success"});
    const notifyError = () => toast("Houve algum erro.", {type: "error"});

    if (result != 0 || result != null) {
      notifySucess()
      const obj = {selectedTipoBem, selectedEstadoName, selectedCidade, selectedDataAquisicao, selectedValorMercado, result};
          ...
          <div className="ml-auto mr-auto bg-white p-6 text-center sm:rounded-3xl lg:w-9/12">
            <form className="items-center gap-4 p-12 sm:grid">
              <div className="mb-4">
                ...
                <select className="w-full rounded border px-3 py-2" onChange={(e) => setSelectedTipoBem(e.target.value)}>
                  ...
                  <option value="Fazenda/Sítio/Chácara">Fazenda/Sítio/Chácara</option>
